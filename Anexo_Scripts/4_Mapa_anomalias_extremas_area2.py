...
ifile = xr.open_dataset('anomalias_tipo_1_anms.nc') # Se carga el archivo NetCDF
...
xsel = Temp2m.sel(time=slice('2013-11-01','2015-09-01'))
...
for kki in range(len(xdates)):
    ...
    niveles_Temp2m = np.arange(-2,2,0.2) #Indicamos los valores que tomará la variable y el intervalo de paso
    mapa = plt.contourf(lon_Temp2m, lat_Temp2m, xsel[kki,:,:],niveles_Temp2m,cmap="RdBu",extend='both') #Pintamos el mapa
...
# =============================================================================
    ...
    plt.title('Anomalía de temperatura a 2m (°C) ' + 'año ' + anio + ' mes ' + mes, size=20, loc='center', pad=8) #Agregamos título al mapa
